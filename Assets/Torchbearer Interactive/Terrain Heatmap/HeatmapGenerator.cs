...
namespace TerrainHeatmap
{
    public class HeatmapGenerator
    {
        ...
        public void GenerateHeatMap(int selectedTextureIndex, ref List<Heatmap> visualisationTextures, int alphaMapResolution, Vector3 terrainObjectSize, int x, int y, int width, int height, float[,] heightMap, Vector3 heightMapScale, bool splatMapUpdateOverride, HeatmapNode[] customDataList, Vector3 positionOffset)
        {
            ...
            if (visualisationTextures == null)
            {
                return;
            }
            else
            {
                dataTexture = visualisationTextures[selectedTextureIndex];
            }




            float originalUpperValueThreshold = dataTexture.upperValueThreshold;
            float originalLowerValueThreshold = dataTexture.lowerValueThreshold;

            if ((dataTexture.texSource == TextureSource.Custom) && (dataTexture.dataVisualisaitonSplatMaps.Count <= 0))
            {
                return;
            }

            if (dataTexture.displayFlippedConstraints) dataTexture = this.FlipDataTextureConstraints(selectedTextureIndex, ref visualisationTextures);

            // Generate SplatPrototypes for heatMap.
            if (!splatMapUpdateOverride)
            {
                if (dataTexture.splatPrototypes == null || dataTexture.splatPrototypesUpdated)
                {
                    if (!isGenerateHeatMapThreadFinished) return;
                    dataTexture.splatPrototypes = GenerateSplatPrototypes(dataTexture);
                }
            }
            else
            {
                dataTexture.splatPrototypes = GenerateSplatPrototypes(dataTexture);
                dataTexture.alphaMapData = new float[alphaMapResolution, alphaMapResolution, dataTexture.splatPrototypes.Length];
            }

            if (!dataTexture.displayFlippedConstraints) dataTexture.coldHotValueOffset = CalculateLowerUpperThresholdValueOffset(dataTexture.lowerValueThreshold);

            // Create array that will be the heatmap splatPrototypes information.
            if (dataTexture.alphaMapData == null)
            {
                dataTexture.alphaMapData = new float[alphaMapResolution, alphaMapResolution, dataTexture.splatPrototypes.Length];
            }
            ...
            dataTexture.visualisationValueMap = new float[alphaMapResolution, alphaMapResolution];

            // Load Genereated heatMap data into Array.
            AssignMapData(dataTexture, alphaMapResolution, terrainObjectSize, heightMap, heightMapScale, customDataList, positionOffset);

            if (dataTexture.autoConstrain) AutoConstrainValues(ref dataTexture, dataTexture.displayFlippedConstraints);
            if (dataTexture.displayFlippedConstraints) dataTexture.coldHotValueOffset = CalculateLowerUpperThresholdValueOffset(dataTexture.lowerValueThreshold);


            // Interpolate the data on the Heat value Map.
            dataTexture.visualisationValueMap = InterpolateColorTextureValues(ref dataTexture, alphaMapResolution);

            // Assignt the correct HeatMap Color value to each pixel on the Terrain.
            AssignVisualisationTextureColorValue(ref dataTexture, x, y, width, height);

            dataTexture.lowerValueThreshold = originalLowerValueThreshold;
            dataTexture.upperValueThreshold = originalUpperValueThreshold;
        ...
        void TerrainHeightToVisualisationTextureData(Heatmap dataTexture, Vector3 terrainObjectSize, int alphaMapResolution, float[,] heightMap, Vector3 heightMapScale)
        {
            ...
            for (int x = 0; x <= dataTexture.heatmapResolution; x++)
            {
                for (int y = 0; y <= dataTexture.heatmapResolution; y++)
                {
                    ...
                    returnVal[x, y] = new HeatmapDatum(nodeLocation, nodeValue, Coordinates.WorldToTerrainCoords(nodeLocation, terrainObjectSize, alphaMapResolution));
                }
            }
        ...
        void CustomDataToVisualisationTextureData(Heatmap dataTexture, HeatmapNode[] customData, bool addCloseValues, Vector3 terrainSize, int alphaMapResolution, Vector3 positionOffset)
        {
            ...
            foreach (HeatmapNode customDatum in customData)
            {
                ...
                HeatmapDatum textureDatum = new HeatmapDatum(closetMapPointPosition, value, new Pair<int>((int)textureCoordianates.x, (int)textureCoordianates.y));
